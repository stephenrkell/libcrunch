...
int initialize_handle(void)
{
	...
	const char *uniqtypes_base_val = getenv("UNIQTYPES_BASE");
	...
	unsigned uniqtypes_base_len = strlen(uniqtypes_base);
	...
	if (!__uniqtypes_handle) return -1;
}
...
int init_allocsites_memtable(void);
int init_allocsites_memtable(void)
{
